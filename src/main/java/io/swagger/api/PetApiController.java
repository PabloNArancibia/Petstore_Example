...
public class PetApiController implements PetApi {
    ...
    public ResponseEntity<ModelApiResponse> addPet(@Parameter(in = ParameterIn.DEFAULT, description = "Optional description in *Markdown*", required=true, schema=@Schema()) @Valid @RequestBody Pet body) {

    	ModelApiResponse response = new ModelApiResponse();
    	
            try {
				log.info("pet :" + body.getName());


					int r= petService.agregarMascota(body);
					
					if(r==1) {
					response.setCode(200);
					response.setMessage("Succesfull, pet has been added");
					response.setType("Succesfull");
					}
					else {
						response.setCode(400);
						response.setMessage("An  error has ocurred");
						response.setType("Error");
						 log.error("Status doesnt match n/json");
						 return new ResponseEntity<ModelApiResponse>(response,HttpStatus.BAD_REQUEST);
					}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				log.error("Error guardando Mascota", e);
    ...
    public ResponseEntity<List<Pet>> findPetsByStatus(@NotNull @Parameter(in = ParameterIn.QUERY, description = "Status values that need to be considered for filter" ,required=true,schema=@Schema(allowableValues={ "dead", "life" }
